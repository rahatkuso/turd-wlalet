...
export class Daemon {
    ...
    checkVersion() {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                let turdd_path = path.join(__turd_bin, "turdd.exe")
                ...
                let turdd_path = path.join(__turd_bin, "turdd")
                let turdd_version_cmd = `"${turdd_path}" --version`
                if (!fs.existsSync(turdd_path))
                    resolve(false)
    ...
    start(options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (process.platform === "win32") {
                this.daemonProcess = child_process.spawn(path.join(__turd_bin, "turdd.exe"), args)
            } else {
                this.daemonProcess = child_process.spawn(path.join(__turd_bin, "turdd"), args, {
                    detached: true
                })
            }
