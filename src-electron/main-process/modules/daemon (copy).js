    ...
    checkVersion() {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                let turdd_path = path.join(__turd_bin, "shit-daemon.exe")
                let turdd_version_cmd = `"${turdd_path}" --version`
                if (!fs.existsSync(turdd_path))
                    resolve(false)
                child_process.exec(turdd_version_cmd, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
            } else {
                let turdd_path = path.join(__turd_bin, "shit-daemon")
                let turdd_version_cmd = `"${turdd_path}" --version`
                if (!fs.existsSync(turdd_path))
                    resolve(false)
                child_process.exec(turdd_version_cmd, {detached: true}, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
    ...
    start(options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(options.app.testnet) {
                this.testnet = true
                args.push("--testnet")
                args.push("--log-file", path.join(options.app.data_dir, "testnet", "logs", "turdd.log"))
            } else {
                args.push("--log-file", path.join(options.app.data_dir, "logs", "turdd.log"))
            }
            ...
            if (process.platform === "win32") {
                this.daemonProcess = child_process.spawn(path.join(__turd_bin, "shit-daemon.exe"), args)
            } else {
                this.daemonProcess = child_process.spawn(path.join(__turd_bin, "shit-daemon"), args, {
                    detached: true
                })
            }
