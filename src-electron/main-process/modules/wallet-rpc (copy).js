...
export class WalletRPC {
        ...
        return new Promise((resolve, reject) => {
            ...
            crypto.randomBytes(64+64+32, (err, buffer) => {
                ...
                if (process.platform === "win32") {
                    this.walletRPCProcess = child_process.spawn(path.join(__turd_bin, "turd-wallet-rpc.exe"), args)
                } else {
                    this.walletRPCProcess = child_process.spawn(path.join(__turd_bin, "sumo-wallet-rpc"), args, {
                        detached: true
                    })
                }
    ...
    listWallets(legacy=false) {
        ...
        if(legacy) {
	        ...
	        legacy_paths = ["C:\\ProgramData\\TurdGUIWallet", "C:\\ProgramData\\TurdLITEWallet"]
            } else {
	        legacy_paths = [path.join(os.homedir(), "TurdGUIWallet"), path.join(os.homedir(), "TurdLITEWallet")]
            }
